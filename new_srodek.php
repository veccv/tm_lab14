        ...
        <div class="container-fluid p-4 text-center">
            ...
            if ($session) {
                ...
                <div style="background-color: white">
                    <form method="POST" class="needs-validation" novalidate action="add_srodek.php"
                          enctype="multipart/form-data">
                        <h2>Nowy środek</h2>
                        <div style="padding: 20px 25%">
                            <label for="lastName" class="form-label">Nazwa środku</label>
                            <input type="text" class="form-control" name="nazwaSrodka" id="nazwaSrodka"
                                   placeholder="Nazwa środku"
                                   value=""
                                   required>
                        </div>
                        <div style="padding: 20px 25%">
                            <label for="avatar" class="form-label">Ikona:</label>
                            <input type="file" name="fileToUpload" id="fileToUpload" required">
                        </div>
                        <?php
                        echo '<input type="hidden" name="id" id="id" value="' . $podkladId . '">';
                        ?>
                        <button class="w-40 btn btn-success btn-lg p-12" type="submit">Stwórz nowy środek
                        </button>
                    </form>
                </div>
                <?php
